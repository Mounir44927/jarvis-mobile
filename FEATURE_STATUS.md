...
|---------|--------|-----------|----------------|
| 2026-09-19 | CI #10 فشل في الترجمة (`compileDebugUnitTestKotlin`): فرعان في `when` من اختبارات Phase 9 استخدما `org.junit.Assert.fail()` — دالة Java تعيد void/Unit فصار نوع التعبير Any بدل List‹Evidence› | استبدالهما بـ `error()` من مكتبة Kotlin القياسية — تعيد Nothing فيُحتفظ بنوع التعبير | CI #11 (المتوقع) |
