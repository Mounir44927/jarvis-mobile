...
| 7 | TTS + ArabicTextNormalizer | BUILD PASS (البنية الأساسية) — `tts/`: **JarvisVoiceSpec** (شخصية رسمية بحدود pitch/rate/rasp مُفرضة — رجولي عميق هادئ بلا مبالغة) + **ArabicTextNormalizer** (جدول تطبيع كامل: أرقام حتى 999M، عشرية، شرقية، عملات، نسب، عمليات، اختصارات، لاتيني شائع، ترقيم، تشكيل + SAFE FAILURE حتمي) + **TtsFallbackChain** (NEURAL→CLOUD→SYSTEM بترتيب مُفرض بنيوياً: تبدأ بـNEURAL دائماً — System لا يكون أساساً أبداً + fallback تلقائي + SafeFailure بلا استثناء). تحقق محلي: **109/109** (36 صوتياً جديدة). يُعتمد بعد: ربط محركات فعلية (sherpa-onnx/System) + CI أخضر + تقييم مسموع على جهاز حقيقي ضد §3.1 |
